...
def admin_dashboard():
    ...
    reported_items = Item.query.all()
    lost_items = Item.query.filter_by(status='Found').all()

    print("Users:", users)  # Debugging
    print("Reported Items:", reported_items)
    print("Lost Items:", lost_items)

    return render_template('admin_dashboard.html', users=users, reported_items=reported_items, lost_items=lost_items)  # Render the admin dashboard page
